...
void ofxFFmpegRecorder::setAudioOutputPath(const std::string& path)
{
	if (isRecording()) {
		LOG_NOTICE("A recording is in proggress. The change will take effect for the next recording session.");
	}

	m_AudioOutputPath = path;
}
...
bool ofxFFmpegRecorder::record(float duration)
{
	...
	if (m_IsRecordAudio) {
		...
		std::string audio_name = "Analogue 1 + 2 (Focusrite Usb Audio)";
		inputDevices += "audio=\"" + audio_name + "\"";
...
bool ofxFFmpegRecorder::startCustomRecord()
{
	...
	std::string cmdAudio = m_FFmpegPath + " -f dshow -y -i audio=\"Analogue 1 + 2 (Focusrite Usb Audio)\" " + m_AudioOutputPath;
	std::cout << "cmdAudio: " << cmdAudio << std::endl;

#if defined(_WIN32)
	m_CustomRecordingFile = _popen(cmd.c_str(), "wb");
	m_CustomRecordingFileAudio = _popen(cmdAudio.c_str(), "w");
#else
...
void ofxFFmpegRecorder::stop()
{
	if (m_CustomRecordingFile || m_CustomRecordingFileAudio) {
		...
		std::string quit = "q"; 
		fwrite(quit.c_str(), sizeof(char), 1, m_CustomRecordingFileAudio);
...
void ofxFFmpegRecorder::determineDefaultDevices()
{
	...
	if (m_IsRecordAudio && m_DefaultAudioDevice.isDefaultInput == false) {
		...
		auto device = soundStream.getMatchingDevices("Focusrite USB ASIO");
		m_DefaultAudioDevice = device[0];
...
{
	while (isRecording() && m_CustomRecordingFile) {
